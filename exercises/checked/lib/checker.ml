...
let rec chk_expr : expr -> texpr tea_result = function 
  ...
  | Unpair(id1,id2,e1,e2) -> (*e1 and e2 are expressions*)
    chk_expr e1 >>= fun te1 ->
    (match te1 with
    | PairType (t1, t2) -> 
      extend_tenv id1 t1 >>+
      extend_tenv id2 t2 >>+
      chk_expr e2 (*just check the type of expression e2. if bool + int, error. if int + int, Ok Int.*)
    | __ -> error "unpair: target must be a pair"
    )
  | Debug(_e) ->
