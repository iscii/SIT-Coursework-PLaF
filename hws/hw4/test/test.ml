...
let tests_erefs_mf = [
  "ref1"  >:: (fun _ -> assert_equal (Ok (BoolVal true))
                 (interp "number?(2)"));

  "ref2"  >:: (fun _ -> assert_equal (Ok (BoolVal true))
                 (interp "number?(2+1)"));
  
  "ref3"  >:: (fun _ -> assert_equal (Ok (BoolVal false))
                  (interp "number?(zero?(0))"));

  "ref4"  >:: (fun _ -> assert_equal (Ok (BoolVal false))
                  (interp "number?(proc(x) {x+1})"));

  "ref4"  >:: (fun _ -> assert_equal ~cmp:(fun x y
                                            -> match x,y with Error_,Error _ -> true |
                                              _,_ -> false) (Error "Field not mutable")
                  (interp "  let p = { ssn = 10; age <= 20} 
in begin
    p.ssn <= 11;
    p.age
   end"));
...
let _ = run_test_tt_main ("suite" >::: (tests_let @ tests_proc
                                        @ tests_rec @ tests_erefs @ tests_erefs_mf))
